require.config({
    paths: {
        ...
        'vue-fullcalendar': 'http://5a1f136e6e6f6472c6240000.codecloudapp.com/vue-fullcalendar',
...
require(['Vue', 'vuex', 'vue2-filters', 'vue_router', 'routes', 'datastore', 'vue-i18n', 'locales', 'moment', 'vue-meta', "moment-timezone", "vue-moment", 'vue-social-sharing', 'vue-select', "vue!search-component", 'vue-fullcalendar', 'vue-simple-spinner', 'vue!loader.vue', 'vue!header', 'json!menu_items.json'], function(Vue, Vuex, Vue2Filters, VueRouter, appRoutes, store, VueI18n, messages, moment, Meta, tz, VueMoment, SocialSharing, VueSelect, SearchComponent, FullCalendar, Spinner, Loader, Header, MenuItems) {
    Vue.use(Meta);
    Vue.use(VueRouter);
    Vue.use(Vue2Filters);
    Vue.use(VueI18n);
    ...
    const vm = new Vue({
        ...
        created() {
            this.loadData().then(response => {
                ...
                this.property.default_logo_url = "//codecloud.cdn.speedyrails.net/sites/5adf56af6e6f6412bb160000/image/jpeg/1515531874445/canyon_crest_default.jpg";
                this.magazine_url;
            });
        ...
        methods: {
            loadData: async function() {
                try {
                    ...
                    let results = await Promise.all([this.$store.dispatch("INITIALIZE_LOCALE"), this.$store.dispatch("getData", "hours"), this.$store.dispatch("getData", "stores"), this.$store.dispatch("getData", "repos"), this.$store.dispatch("LOAD_META_DATA")]);
